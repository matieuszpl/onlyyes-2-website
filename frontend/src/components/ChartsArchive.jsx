...
export default function ChartsArchive() {
  ...
  const loadCharts = async () => {
    try {
      const res = await api.get(`/charts?period=${selectedPeriod}`);
      setCharts(res.data);
    } catch (error) {
          ...
          {charts.map((chart, idx) => {
            ...
            return (
              ...
              >
                ...
                {chart.thumbnail && (
                  <div className="w-12 h-12 border border-primary/50 rounded shrink-0 overflow-hidden relative z-10">
                    <ImageGlitch
                      src={chart.thumbnail}
                      alt={chart.title}
                      shouldGlitch={false}
